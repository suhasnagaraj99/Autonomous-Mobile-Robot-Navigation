#include "tbot_follow_waypoints.h"

//Function to set initial pose 
void turtle::TurtleWaypoints::set_initial_pose() {
...
void turtle::TurtleWaypoints::goals_cb(geometry_msgs::msg::PoseArray msg){
    wp1_=msg.poses[0];
    wp2_=msg.poses[1];
    wp3_=msg.poses[2];
    wp4_=msg.poses[3];
...
void turtle::TurtleWaypoints::send_goals() {
  ...
  RCLCPP_INFO(this->get_logger(), "Sending goal");
    ...
    send_goal_options.goal_response_callback =
        std::bind(&TurtleWaypoints::goal_response_callback, this, _1);
    send_goal_options.feedback_callback =
        std::bind(&TurtleWaypoints::feedback_callback, this, _1, _2);
    send_goal_options.result_callback =
        std::bind(&TurtleWaypoints::result_callback, this, _1);
...
void turtle::TurtleWaypoints::goal_response_callback(const rclcpp_action::ClientGoalHandle<nav2_msgs::action::FollowWaypoints>::SharedPtr & goal_handle)
{
  if (!goal_handle) {
...
void turtle::TurtleWaypoints::feedback_callback(
...
void turtle::TurtleWaypoints::result_callback(
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<turtle::TurtleWaypoints>("tbot_nav");
  rclcpp::spin(node);
  rclcpp::shutdown();
}
